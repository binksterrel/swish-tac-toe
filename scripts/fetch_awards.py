...
def fetch_awards():
    ...
    def save_progress():
        ...
        try:
            ...
            print(f"✅ Progress saved safely to {DATA_FILE}")
        except Exception as e:
            print(f"❌ Error saving progress: {e}")

    # Signal handling for safe exit
    def signal_handler(sig, frame):
        print(f"\n\n🛑 Aggressive termination signal received ({sig}). Saving progress...")
        save_progress()
        sys.exit(0)
    ...
    try:
        ...
        for i, player in enumerate(players):
            # START_INDEX Logic (Manual Override)
            # You can uncomment and set this if you want to skip purely by index (risky if list changes)
            # if i < 1920: continue 
