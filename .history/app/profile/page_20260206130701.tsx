                ...
                <div className="mb-8 md:mb-12 flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div>
                        <h1 className="text-4xl md:text-6xl font-heading font-bold italic tracking-tighter text-white uppercase drop-shadow-2xl">
                            Profil <span className="text-transparent bg-clip-text bg-gradient-to-r from-gray-200 to-gray-500"></span>
                        </h1>
                                ...
                                <h2 className="text-3xl font-heading font-bold italic text-white uppercase tracking-tighter mb-1 relative inline-block">
                                    ...
                                    {favoriteTeam && (
                                        <div className="absolute -top-4 -right-8 w-8 h-8 opacity-80" title={favoriteTeam.name}>
                                              <TeamLogo teamId={favoriteTeamId} size={32} className="drop-shadow-md w-full h-full" />
                                        </div>
                                    )}
                                </h2>
                             ...
                             <AnimatePresence mode="wait">
                                ...
                                {activeTab === 'cabinet' && (
                                    ...
                                    >
                                        ...
                                        {(() => {
                                            const trophyCtx = stats ? buildTrophyContext(stats) : null
                                            const trophies = trophyCtx ? evaluateTrophies(trophyCtx) : []
                                            const unlockedCount = trophies.filter(t => t.unlocked).length

                                            const ICON_MAP: Record<string, React.ComponentType<{ className?: string }>> = {
                                                Trophy, Medal, Star, Crown, Flame, Zap, Swords, Shield, Eye, Target
                                            }
                                            ...
                                            return (
                                                <div className="space-y-6">
                                                    ...
                                                    <div className="flex items-center justify-between bg-black/20 rounded-xl px-4 py-3 border border-white/5">
                                                        <span className="text-xs text-gray-400 uppercase tracking-widest font-bold">Unlocked</span>
                                                        <span className="text-sm font-bold font-heading text-white">{unlockedCount} / {trophies.length}</span>
                                                    </div>

                                                    {TROPHY_CATEGORIES.map(cat => {
                                                        const catTrophies = trophies.filter(t => t.category === cat.id)
                                                        if (catTrophies.length === 0) return null
                                                        return (
                                                            <div key={cat.id}>
                                                                <h4 className="text-[10px] uppercase tracking-widest font-bold text-gray-500 mb-3 ml-1">{cat.label}</h4>
                                                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                                                    {catTrophies.map(trophy => {
                                                                        const IconComp = ICON_MAP[trophy.icon] || Trophy
                                                                        return (
                                                                            <div key={trophy.id} className={cn(
                                                                                "rounded-2xl border flex flex-col items-center justify-center p-4 text-center group transition-all hover:scale-105 relative overflow-hidden",
                                                                                trophy.unlocked
                                                                                    ? `${trophy.bgColor} ${trophy.borderColor}`
                                                                                    : "border-white/5 bg-white/[0.02] opacity-50 grayscale"
                                                                            )}>
                                                                                <IconComp className={cn("w-7 h-7 mb-2", trophy.unlocked ? trophy.color : "text-gray-600")} />
                                                                                <div className={cn("text-[11px] font-bold uppercase tracking-wider mb-0.5", trophy.unlocked ? "text-white" : "text-gray-500")}>{trophy.label}</div>
                                                                                <div className="text-[9px] text-gray-500 leading-tight">{trophy.description}</div>
                                                                                {!trophy.unlocked && (
                                                                                    <>
                                                                                        <div className="w-full mt-2 h-1 bg-white/5 rounded-full overflow-hidden">
                                                                                            <div className="h-full bg-white/20 rounded-full transition-all" style={{ width: `${trophy.progress}%` }} />
                                                                                        </div>
                                                                                        <Lock className="w-2.5 h-2.5 text-gray-600 mt-1.5" />
                                                                                    </>
                                                                                )}
                                                                            </div>
                                                                        )
